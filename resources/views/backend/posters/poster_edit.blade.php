...
@section('content')
    <form action="{{ route('backend.poster.edited', ['id' => $poster_group->id()]) }}" method="post">
        @csrf
                ...
                <div class="tab-pane active show" id="tab1" role="tabpanel" aria-expanded="true">
                    <div class="col-lg-6 col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" value="{{ old('name', $poster_group->name) }}" class="form-control" id="name"/>
                        </div>
                        <div class="form-group">
                            <label for="name">Açıklama</label>
                            <textarea name="description" class="form-control" id="description">{{ old('description', $poster_group->description) }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="status">Durum</label>
                            <select name="status" id="status" class="form-control">
                                <option @if(old('status', $poster_group->status) == "0") selected
                                        @endif value="0">@lang('backend/common.status_0')</option>
                                <option @if(old('status', $poster_group->status) == "1") selected
                    ...
                    <div class="col-lg-12 col-md-12">
                        ...
                        <div class="form-group">
                            <table class="table table-striped table-bordered" id="table-poster">
                                ...
                                <tbody>
                                @if(old('poster', $poster_group->posters->count()))
                                @foreach(old('poster', $poster_group->posters) as $i => $poster)
                                    <tr>
