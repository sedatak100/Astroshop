...
@section('content')
    <div class="main-container">
        <form action="{{ route('backend.user.group.edited', ['id' => $user_group->id()]) }}" method="post">
            @csrf
            <div class="col-lg-8 col-md-8">
                <div class="form-group">
                    <label for="name">Yetki Adı</label>
                    <input type="text" name="name" value="{{ old('name', $user_group->name) }}" class="form-control" id="name" required />
                </div>
                <div class="form-group">
                                ...
                                @foreach($permission['roles'] as $role)
                                    <div class="col-lg-4">
                                        <input type="checkbox"
                                               ...
                                               @if(in_array($role['routes'][0], $user_group->permissions()) !== false) checked @endif
