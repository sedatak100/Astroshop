...
@section('content')
    <div class="main-container">
        <form action="{{ route('backend.region.edited', ['id' => $region->id()]) }}" method="post">
            @csrf
            <div class="col-lg-6 col-md-6">
                <div class="form-group">
                    <label for="name">Adı</label>
                    <input type="text" name="name" value="{{ old('name', $region->name) }}" class="form-control" id="name"/>
                </div>
                <div class="form-group">
                    <label for="description">Açıklama</label>
                    <input type="text" name="description" value="{{ old('description', $region->description) }}" class="form-control" id="description"/>
                </div>
            </div>
            <div class="col-lg-10 col-md-10">
                <table class="table table-bordered table-striped" id="table-scope">
                    ...
                    <tbody>
                    @if(old('country_ids', $selected_country))
                        @foreach(old('country_ids', $selected_country) as $i => $country_id)
                            <tr>
                                <th>
                                ...
                                <th>
                                    <select name="city_ids[]" class="form-control select-city selectbox" data-selected_id="{{ old('city_ids.' . $i, $selected_city[$i]) }}">
                                    </select>
                                </th>
                                <th>
