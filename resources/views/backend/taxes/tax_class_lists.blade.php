    ...
    <div class="main-container ">
        <div class="table-header">
            ...
            <div class="table-header__controls">
                <a href="{{ route('backend.tax.class.add') }}" class="btn btn-success icon-left mr-3">
        ...
        @if($tax_classes->count())
            <table class="table table-bordered table-striped">
                ...
                @foreach($tax_classes as $tax_class)
                    <tr>
                        <td>{{ $tax_class->name }} @if($tax_class->isDefault())<strong>(@lang('backend/common.default'))</strong>@endif</td>
                        <td>{{ $tax_class->description }}</td>
                        <td>{{ $tax_class->updatedAt() }}</td>
                        <td>
                            <a href="{{ route('backend.tax.class.edit', ['id' => $tax_class->id()]) }}"
                               class="btn btn-primary" data-toggle="tooltip" title="@lang('backend/common.edit')">
                                <span class="ua-icon-pencil"></span>
                            </a>
                            <a href="{{ route('backend.tax.class.remove', ['id' => $tax_class->id()]) }}"
