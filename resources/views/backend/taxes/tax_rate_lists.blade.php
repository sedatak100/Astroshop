    ...
    <div class="main-container ">
        <div class="table-header">
            ...
            <div class="table-header__controls">
                <a href="{{ route('backend.tax.rate.add') }}" class="btn btn-success icon-left mr-3">
            ...
            <table class="table table-bordered table-striped">
                ...
                @foreach($tax_rates as $tax_rate)
                    <tr>
                        <td>{{ $tax_rate->name }}</td>
                        <td>{{ $tax_rate->rate }}</td>
                        <td>{{ $tax_rate->region()->name }}</td>
                        <td>@lang('backend/common.tax_rate_type_' . $tax_rate->type)</td>
                        <td>{{ $tax_rate->updatedAt() }}</td>
                        <td>
                            <a href="{{ route('backend.tax.rate.edit', ['id' => $tax_rate->id()]) }}"
                               class="btn btn-primary" data-toggle="tooltip" title="@lang('backend/common.edit')">
                                <span class="ua-icon-pencil"></span>
                            </a>
                            <a href="{{ route('backend.tax.rate.remove', ['id' => $tax_rate->id()]) }}"
