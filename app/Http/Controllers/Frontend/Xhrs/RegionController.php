...
namespace App\Http\Controllers\Frontend\Xhrs;
...
class RegionController extends FrontendController
{
    ...
    public function districtCityCountry(Request $request)
    {
        $data = [];
        $id = $request->input('id');
        $district = District::where('status', 1)->findOrFail($id);
        $city = $district->city(1);
        $country = $district->country(1);
        if ($city && $country) {
            $data = [
                'country' => [
                    'id' => $country->id(),
